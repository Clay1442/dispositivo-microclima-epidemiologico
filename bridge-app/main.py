...
influx_client = InfluxDBClient(url=INFLUX_URL, token=INFLUX_TOKEN, org=INFLUX_ORG)  # client InfluxDB para gravar dados no bucket
write_api = influx_client.write_api(write_options=SYNCHRONOUS) # api de escrita que envia os dados imediatamente 

#cria um ponto de dados para ser armazenado 
...
# callback que executa automaticamente sempre que o cliente MQTT recebe uma nova mensagem de um tópico inscrito
...
# Verifica se este arquivo está sendo executado diretamente (e não importado como módulo em outro arquivo)
